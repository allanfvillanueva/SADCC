...
public class ClientNotificationsActivity extends  AppCompatActivity {
    ...
    private HashMap<String, Object> notificationmap = new HashMap<>();
    private boolean isAdmin = false;
    private  DatabaseReference notifdb;
    private HashMap<String, Object> MarkAsReadNotificationMap = new HashMap<>();

    private ArrayList<HashMap<String, Object>> notificationml = new ArrayList<>();
    private ArrayList<HashMap<String, Object>> newNotificationMl = new ArrayList<>();
    private ArrayList<String> notificationstring = new ArrayList<>();
    ...
    private DatabaseReference dbnotification = _firebase.getReference("notification");
    private ChildEventListener _dbnotification_child_listener;
    ...
    private DatabaseReference fbnotifications = _firebase.getReference("notification");
    private ChildEventListener _fbnotifications_child_listener;

    private ArrayList<HashMap<String, Object>> notificationslm = new ArrayList<>();
    ...
    private ArrayList<String> notificationmapliststring = new ArrayList<>();

    public void _loadAllNotifications () {
        fbnotifications.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot _dataSnapshot) {
                notificationslm = new ArrayList<>();
                try {
                    GenericTypeIndicator<HashMap<String, Object>> _ind = new GenericTypeIndicator<HashMap<String, Object>>() {};
                    for (DataSnapshot _data : _dataSnapshot.getChildren()) {
                        HashMap<String, Object> _map = _data.getValue(_ind);
                        notificationslm.add(_map);
                        Log.d("av","loadAllNotifications notificationslm.size()=" + notificationslm.size());
                    }
                }
                catch (Exception _e) {
                    _e.printStackTrace();
                }
                allnotificationlm.clear();
                index = 0;
                length = notificationslm.size();
                for(int _repeat14 = 0; _repeat14 < (int)(length); _repeat14++) {
                    notificationmapliststring.clear();
                    SketchwareUtil.getAllKeysFromMap(notificationslm.get((int)(index)), notificationmapliststring);
                    index1 = 0;
                    length1 = notificationmapliststring.size();

                    Log.d("av","loadAllNotifications notificationmapliststring.size()=" + notificationmapliststring.size());

                    for(int _repeat31 = 0; _repeat31 < (int)(length1); _repeat31++) {
                        allnotificationlm.add((HashMap<String,Object>)notificationslm.get((int)(index)).get(notificationmapliststring.get((int)(index1))));
                ...
                for(int index3 = 0; index3 < (int)(allnotificationlm.size()); index3++) {
                    Log.d("av","loadAllApointments " + allnotificationlm.get((int)(index3)).get("date").toString().toUpperCase() + " " + allnotificationlm.get((int)(index3)).get("time").toString().toUpperCase());
                }
    ...
    protected void onCreate(Bundle _savedInstanceState) {
        ...
        Log.d("av","ClientNotificationsActivity onCreate");
    ...
    private void initialize(Bundle _savedInstanceState) {
        ...
        _dbnotification_child_listener = new ChildEventListener() {
            @Override
        ...
        dbnotification.addChildEventListener(_dbnotification_child_listener);
    ...
    private void initializeLogic() {
        ...
        dbaccount.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot _dataSnapshot) {
                ...
                clientName = AccountInfoMap.get("lname").toString() + ", " + AccountInfoMap.get("fname").toString();
                ...
                if (AccountInfoMap.containsKey("account_type")) {
                    ...
                    SketchwareUtil.showMessage(getApplicationContext(), "Failed to load some of your data, Please try again.");
                    finish();
                }
            }
            @Override
    ...
    public void _LoadAllNotification (final boolean _isAdmin, final String _userid) {
        isAdmin = _isAdmin;
        if (_isAdmin) {
            dbnotifadmin.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot _dataSnapshot) {
                    ...
                    try {
                        ...
                        for (DataSnapshot _data : _dataSnapshot.getChildren()) {
                            ...
                            if (_map.get("message").toString().contains(clientName)) {
                    ...
                    lv.setAdapter(new ClientNotificationsActivity.LvAdapter(notificationml));
                    ((BaseAdapter)lv.getAdapter()).notifyDataSetChanged();
                }
                @Override
                public void onCancelled(DatabaseError _databaseError) {
    ...
    public class LvAdapter extends BaseAdapter {
        ...
        public View getView(final int _position, View _v, ViewGroup _container) {
            ...
            View _view = _v;
            if (_view == null) {
                _view = _inflater.inflate(R.layout.lv_notification, null);
            }
            ...
            final Button btnMarkAsRead = (Button) _view.findViewById(R.id.btnMarkAsRead);
            ...
            android.graphics.drawable.GradientDrawable btnMarkAsRead_design = new android.graphics.drawable.GradientDrawable();
            btnMarkAsRead_design.setColor(0xFFDD1D5E);
            btnMarkAsRead_design.setCornerRadius((float)8);
            btnMarkAsRead_design.setStroke((int)0,Color.TRANSPARENT);
            android.graphics.drawable.RippleDrawable btnMarkAsRead_re = new android.graphics.drawable.RippleDrawable(new android.content.res.ColorStateList(new int[][]{new int[]{}}, new int[]{ 0xFFFCE4EC }), btnMarkAsRead_design, null);
            btnMarkAsRead.setBackground(btnMarkAsRead_re);
            btnMarkAsRead.setVisibility(View.GONE);

            // Bind
            description.setText(_data.get((int)_position).get("message").toString());
